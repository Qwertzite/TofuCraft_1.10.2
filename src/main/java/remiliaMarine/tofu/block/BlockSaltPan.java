...
import remiliaMarine.tofu.versionAdapter.BiomeAdapter;
...
public class BlockSaltPan extends TcBlockContainer {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        {
            ItemStack itemHeld = playerIn.inventory.getCurrentItem();
            ...
            if (stat == Stat.EMPTY && itemHeld != null && itemHeld.getItem() == Items.WATER_BUCKET)
            {
                ...
                tileScanner.scan(1, TileScanner.Method.fullSimply, new TileScanner.Impl<Object>()
                {
                    ...
                    public void apply(World world, BlockPos pos)
                    {
                        ...
                        {
                            world.setBlockState(pos, TcBlocks.saltPan.getDefaultState().withProperty(STATUS, Stat.WATER), 3);
                        }
                    }
                });

                worldIn.setBlockState(pos, TcBlocks.saltPan.getDefaultState().withProperty(STATUS, Stat.WATER), 3);
            ...
            else if (stat == Stat.BITTERN && itemHeld != null && itemHeld.getItem() == Items.GLASS_BOTTLE)
            {
                ...
                if (ItemStackAdapter.getSize(itemHeld) == 1)
                {
                    playerIn.inventory.setInventorySlotContents(playerIn.inventory.currentItem, nigari);
                ...
                else
                {
                    ...
                    {
                    	WorldAdapter.spawnEntity(worldIn, new EntityItem(worldIn, (double)pos.getX() + 0.5D, (double)pos.getY() + 1.5D, (double)pos.getZ() + 0.5D, nigari));
                    }
                ...
                worldIn.setBlockState(pos, TcBlocks.saltPan.getDefaultState().withProperty(STATUS, Stat.EMPTY), 3);

                TcAchievementMgr.achieve(playerIn, TcAchievementMgr.Key.bayBittern);
                return true;
            }
            ...
            else if (stat == Stat.SALT)
            {
                ...
                WorldAdapter.spawnEntity(worldIn, entityitem);

                worldIn.setBlockState(pos, TcBlocks.saltPan.getDefaultState().withProperty(STATUS, Stat.BITTERN), 3);

                TcAchievementMgr.achieve(playerIn, TcAchievementMgr.Key.baySalt);
                return true;
            }
            return false;
        }
    ...
    private float calcAdaptation(World world, BlockPos pos)
    {
        ...
        float temperature = BiomeAdapter.getTamperature(biome, pos);
        float rate;
