 ...
 //       this.yOffset = 0.0F;
    ...
    public void onUpdate()
    {
        ...
        if (!this.inGround && this.isCrit)
        {
            for (int l = 0; l < 2; ++l)
            {
                this.worldObj.spawnParticle(EnumParticleTypes.CRIT, this.posX + this.motionX * l / 2.0D, this.posY + this.motionY * l / 2.0D, this.posZ + this.motionZ * l / 2.0D, -this.motionX, -this.motionY + 0.2D, -this.motionZ);
            }
        }
    }
