...
public class MorijioManager {
	...
	public boolean isInfoRegistered(TileEntityMorijio.MorijioInfo info) {
		DataSectorCoord sectorCoord = DataSectorCoord.getFromCoord(info.coord.getX(), info.coord.getZ(), info.coord.dim);
    ...
    public void registerInfo(TileEntityMorijio.MorijioInfo info)
    {
        this.addInfo(info);
        this.modified = true;
        ModLog.debug("Registered morijio info at: (%d, %d, %d), %d left", info.coord.getX(), info.coord.getY(), info.coord.getZ(), morijioInfoList.size());
    }
    ...
    public boolean isInRangeOfMorijio(World world, int tileX, int tileY, int tileZ, int dim)
    {
        ...
        {
            if (Math.abs(tileX - morijio.coord.getX()) <= range
                    && Math.abs(tileY - morijio.coord.getY()) <= range
                    && Math.abs(tileZ - morijio.coord.getZ()) <= range)
            {
                Block block = world.getBlockState(morijio.coord).getBlock();
                ...
                else if (world.isBlockLoaded(morijio.coord))
                {
    ...
    public void removeInfo(TileEntityMorijio.MorijioInfo info)
    {
        ...
        if (list != null && list.contains(info))
        {
            ...
            this.modified = true;
            ModLog.debug("Removed morijio info at: (%d, %d, %d), %d left", info.coord.getX(), info.coord.getY(), info.coord.getZ(), morijioInfoList.size());
        }
    }
